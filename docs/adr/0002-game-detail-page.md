...
一場比賽換過二壘手後，box score 的打線會同時存在兩位標記 `2B` 的球員，但場上同一時間只能有一位。`buildFieldAlignment()` 因此取每個棒次的**最後一位（當前在場者）**，被換下的先發不會留在守備圖上——他仍完整保留在打序分頁的換人鏈中，守備圖上則以「替下 ○○○」註明。

三種實際情境的處理（皆有單元測試鎖定）：

1. **同棒次直接替換**（`400` → `401`，兩人皆 `2B`）：圖上只顯示替補，並標註替下誰。
2. **守備輪轉**（原二壘手移防游擊，新球員進另一棒次守二壘）：兩個位置各自正確，不互相覆蓋。
3. **代打／代跑剛上場**（`position` 仍為 `PH` / `PR`，MLB 尚未指派守位）：若不處理，該守位會在進行中的比賽裡整個空缺。改以**第二輪回填**：該棒次繼承前一位的守位並標記 `isPending`（虛線圈 ＋「守位待定」），且**永不覆蓋**已確認的守備者。
4. **雙重守備變換（double switch）**：打序棒次與守備位置是分開易主的——接手第 4 棒的球員可能去守左外野，而原左外野手移防二壘。因此「替下 ○○○」**僅在前一位確實守過同一位置時才顯示**，否則不標；否則會出現「Duran 替下 Semien（守 LF）」這種 Semien 根本沒守過左外野的假敘述。

> ⚠️ **這些都是單張快照內的消歧義，不是時間軸還原。** boxscore 只給「當前／最終」狀態，因此頁面能回答「現在誰守二壘」「他是替下同位置的誰」「這人本場守過哪些位置（`allPositions`，無局數）」，但**無法**回答「第幾局換的」或「第 3 局當下的守備配置長怎樣」——後者需要 feed/live 的 `playEvents`，屬 v2 範圍。
